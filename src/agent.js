...
const SKILLS_DIR   = join(PROJECT_DIR, 'skills');      // skills.md, memory.md
...
// FILE HELPERS
// ================================================================
// قراءة ملف من skills/ (skills.md, memory.md)
...
// يدمج skills.md + memory.md الحالي + task.md المؤقت
// يُعاد بناؤه في كل round مع أحدث نسخة من memory
// ================================================================
function buildSystemInstruction(skillsMd, currentMemory, convId) {
...
// ================================================================
async function reactLoop(uid, convId, userMsg, history, skillsMd) {
  ...
  for (let round = 0; round < 15; round++) {
    ...
    const sysInst = buildSystemInstruction(skillsMd, currentMemory, convId);
...
async function mainAgent() {
  ...
  // skills.md — ملف موحّد: هوية + تفكير + actions + shell ref
  const skillsMd = readSkill('skills.md');
  if (!skillsMd) { log('error', 'agent', 'skills/skills.md not found'); process.exit(1); }
...
## تعليمات
- نفّذ المهمة كاملاً ومستقلاً — لا تنتظر input
- استخدم Plan-and-Solve: خطّط أولاً ثم نفّذ
- الرد النهائي = ناتج المهمة المباشر (الحكمة، التقرير، البيانات)
- لا تشرح ما ستفعله — نفّذ وأعطِ النتيجة
